...
import { Pencil, Save, XCircle, X, Download, Settings, Eye, FileJson, Plus, Check, Trash2, Search, Users } from "lucide-react";
...
function getAssignedManagerId(profile?: Profile | null) {
  return cleanString(profile?.assignedManagerUserId ?? profile?.createdBy);
}
...
export default function ManagerProfilesPage() {
  ...
  const filteredProfiles = useMemo(() => {
    ...
    if (filterCreatedBy) {
      filtered = filtered.filter((p) => getAssignedManagerId(p) === filterCreatedBy);
    }
    ...
    return filtered;
  }, [assignments, filterAssignedBy, filterCreatedBy, profiles, searchQuery]);
  ...
  const getAssignedManagerLabel = useCallback(
    (profile?: Profile | null) => {
      if (!profile) return "Unassigned";
      if (profile.assignedManagerName) return profile.assignedManagerName;
      const managerId = getAssignedManagerId(profile);
      const manager = managerId ? allUsers.find((item) => item.id === managerId) : null;
      return manager ? getUserDisplayName(manager) : managerId || "Unassigned";
    },
    [allUsers],
  );
  const canManageProfile = useCallback(
    (profile?: Profile | null) => {
      ...
      return getAssignedManagerId(profile) === user.id;
    },
    [user?.id, user?.role],
  );
  ...
  const canManageSelectedProfile = canManageProfile(selectedProfile);
  ...
  const loadProfiles = useCallback(async (authToken: string) => {
    try {
      ...
      const visible =
        user?.role === "MANAGER"
          ? list.filter((profile) => getAssignedManagerId(profile) === user.id)
          : list;
      setProfiles(visible);
      setSelectedId("");
      setDetailOpen(false);
  ...
  useEffect(() => {
    ...
    setCreateForm((prev) => {
      if (prev.assignedManagerUserId) return prev;
      return { ...prev, assignedManagerUserId: user.id };
    });
  }, [addOpen, user?.id]);
  ...
  async function handleSaveProfile() {
    ...
    if (!canManageSelectedProfile) {
      setError("Only admins or the assigned manager can update this profile.");
      return;
    }
    setSaving(true);
    setError("");
  ...
  async function handleTemplateChange(profileId: string, nextTemplateId: string) {
    ...
    if (!canManageProfile(targetProfile)) {
      setError("Only admins or the assigned manager can change a profile template.");
      return;
    }
  ...
  async function handleAssignedManagerChange(nextManagerId: string) {
    if (!selectedProfile || !token || user?.role !== "ADMIN") return;
    if (!nextManagerId || nextManagerId === getAssignedManagerId(selectedProfile)) return;
    ...
    try {
      const updated = await api<Profile>(
        `/profiles/${selectedProfile.id}`,
        {
          method: "PATCH",
          body: JSON.stringify({
            assignedManagerUserId: nextManagerId,
          }),
        },
        token,
      );
      ...
      console.error(err);
      setError("Failed to update assigned manager.");
    } finally {
  ...
  async function handleImportBaseResume(file: File) {
    if (!canManageSelectedProfile) {
      setBaseResumeError("Only admins or the assigned manager can edit this base resume.");
      return;
    }
    setBaseResumeError("");
    try {
  ...
  function updateBaseResumeProfileField(field: "name" | "headline", value: string) {
    setBaseResumeDraft((prev) => {
      const next = normalizeBaseResumeDraft(prev);
  ...
  const baseResumeView = normalizeBaseResumeDraft(baseResumeDraft);
  const selectedProfileManagerId = getAssignedManagerId(selectedProfile);
  const selectedProfileManagerLabel = getAssignedManagerLabel(selectedProfile);
  ...
  const baseResumeDirty = useMemo(
    () => serializeBaseResumeDraft(baseResumeDraft) !== serializeBaseResumeDraft(selectedProfile?.baseResume),
    [baseResumeDraft, selectedProfile],
  );
  const baseResumeLocked = !canManageSelectedProfile || !baseResumeEdit || savingBaseResume;
  const creatingDisabled =
    createLoading ||
    !createForm.displayName.trim() ||
    createForm.displayName.trim().length < 2 ||
    !createForm.assignedManagerUserId.trim();
            ...
            <div className="flex flex-col gap-4 border-b border-slate-200 px-4 py-4">
              <div className="flex items-center justify-between">
                ...
                <button
                  onClick={() => {
                    ...
                    setDetailOpen(false);
                    setSelectedId("");
                    setCreateForm(getEmptyCreateForm(user?.id ?? ""));
                  }}
              ...
              <div className="flex flex-wrap items-center gap-3">
                ...
                >
                  ...
                  {Array.from(new Set(profiles.map((p) => getAssignedManagerId(p)).filter(Boolean))).map((managerId) => {
                    const creator = allUsers.find((u) => u.id === managerId);
                    return (
                      <option key={managerId} value={managerId}>
                        {creator?.userName || creator?.name || managerId}
                      </option>
                    );
                  })}
                </select>
                ...
                <table className="w-full">
                  <thead className="bg-slate-50">
                    <tr>
                      ...
                      <th className="px-4 py-3 text-left text-xs font-semibold uppercase tracking-[0.18em] text-slate-500">Assigned Manager</th>
                  ...
                  <tbody className="divide-y divide-slate-200">
                    {filteredProfiles.map((profile, index) => {
                      ...
                      const managerLabel = getAssignedManagerLabel(profile);
                      const rowManagedByUser = canManageProfile(profile);
                      return (
                        <tr
                          key={profile.id}
                          onClick={() => {
                        ...
                        >
                          ...
                          <td className="px-4 py-3 text-sm text-slate-600">{managerLabel}</td>
                          ...
                          <td className="px-4 py-3 text-sm text-slate-600">
                            {resumeTemplatesLoading ? (
                              ...
                              <select
                                ...
                                disabled={templateSavingId === profile.id || !rowManagedByUser}
                          ...
                          <td className="px-4 py-3">
                            <div className="flex items-center gap-2" onClick={(e) => e.stopPropagation()}>
                              ...
                              <button
                                ...
                                disabled={deleteLoading || !rowManagedByUser}
                                ...
                                title={rowManagedByUser ? "Delete" : "Only admins or the assigned manager can delete"}
                              >
                ...
                <div className="flex items-start justify-between gap-4 mb-6">
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-sm text-slate-600 mt-1">
                      Review manager ownership, bidder access, and the base resume used for tailoring.
                    </p>
                  </div>
                  <div className="flex items-center gap-2">
                    <IconButton
                      ...
                      disabled={deleteLoading || !canManageSelectedProfile}
                    >
                      <Trash2 className="w-4 h-4" />
                ...
                <div className="mt-4 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
                  <div className="mb-3">
                    ...
                    <p className="mt-1 text-sm text-slate-600">
                      Assigned managers can edit their profiles; admins can reassign manager ownership.
                    </p>
                  </div>
                  <div className="grid gap-3 md:grid-cols-2">
                    <div className="rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 text-sm text-slate-800">
                      ...
                      {user?.role === "ADMIN" ? (
                        <select
                          value={selectedProfileManagerId}
                          onChange={(event) => {
                            void handleAssignedManagerChange(event.target.value);
                          }}
                          disabled={managerSaving || managerOptions.length === 0}
                          className="mt-1 w-full rounded-lg border border-slate-200 bg-white px-2 py-1.5 text-sm text-slate-900 outline-none ring-1 ring-transparent focus:ring-slate-300 disabled:cursor-wait disabled:opacity-60"
                        >
                          <option value="" disabled>
                            Select manager
                          </option>
                          {selectedProfileManagerId &&
                          !managerOptions.some((manager) => manager.id === selectedProfileManagerId) ? (
                            <option value={selectedProfileManagerId}>{selectedProfileManagerLabel}</option>
                          ) : null}
                        ...
                        <div className="mt-1 text-slate-900">{selectedProfileManagerLabel || "-"}</div>
                      )}
                    </div>
                    <ReadRow
                      label="Assigned bidder"
                      value={activeAssignment ? (bidders.find((b) => b.id === activeAssignment.bidderUserId)?.userName || "Assigned") : "Unassigned"}
                    />
                  </div>
                  {!canManageSelectedProfile ? (
                    <div className="mt-4 rounded-xl border border-amber-200 bg-amber-50 px-3 py-2 text-xs text-amber-800">
                      Only admins or the assigned manager can edit this profile, delete it, or change bidder access.
                    </div>
                  ) : null}
                </div>
                ...
                <div className="mt-6 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <select
                      ...
                      disabled={!canManageSelectedProfile || assignLoading}
                  ...
                  <p className="mt-3 text-xs text-slate-500">
                    Only admins or the assigned manager can change bidder access.
                  </p>
                </div>
                  ...
                  <div className="mb-4 flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
                    ...
                    <div className="flex flex-wrap items-center gap-2">
                      ...
                      <IconButton
                        ...
                        disabled={savingBaseResume || !canManageSelectedProfile}
                        title="Import JSON"
                      >
                      ...
                      {baseResumeEdit ? (
                        <>
                          <IconButton
                            ...
                            disabled={savingBaseResume || !baseResumeDirty || !canManageSelectedProfile}
                ...
                >
                  <div className="mb-4">
                    ...
                    <h3 className="text-xl font-semibold text-amber-900 mt-1">
                      Change the bidder on {selectedProfile.displayName} from {activeAssignment ? (bidders.find(b => b.id === activeAssignment.bidderUserId)?.userName || "Unknown") : "None"} to {assignBidderIdPending ? (bidders.find(b => b.id === assignBidderIdPending)?.userName || "None") : "None"}?
                    </h3>
                  </div>
              ...
              <div className="flex h-full flex-col overflow-y-auto p-6">
                <div className="flex items-start justify-between gap-4 mb-6">
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-sm text-slate-600 mt-1">
                      Create a profile with a base resume and assigned manager.
                    </p>
                  </div>
                  <IconButton
                ...
                <div className="mt-6 grid gap-3 md:grid-cols-2">
                  ...
                  <div className="space-y-1">
                    <span className="text-xs uppercase tracking-[0.18em] text-slate-600">Assigned manager</span>
                    <select
                      value={createForm.assignedManagerUserId}
                      onChange={(e) =>
                        setCreateForm((f) => ({
                          ...f,
                          assignedManagerUserId: e.target.value,
                        }))
                      }
                      disabled={user?.role !== "ADMIN"}
                      className="w-full rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm text-slate-900 outline-none ring-1 ring-transparent focus:ring-slate-300 disabled:cursor-not-allowed disabled:bg-slate-100 disabled:text-slate-500"
                    >
                      <option value="" disabled>
                        Select manager
                      </option>
                  ...
                  <div className="md:col-span-2 rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-600">
                    The new profile will start with an empty base resume. Managers are assigned to themselves automatically; admins can choose any active manager.
                  </div>
                </div>
  ...
  async function handleAssign(confirmedBidderId?: string) {
    const bidderIdToAssign = confirmedBidderId !== undefined ? confirmedBidderId : assignBidderIdPending;
      ...
      const created = await api<Profile>(
        "/profiles",
        {
          ...
          body: JSON.stringify({
            ...
            createdBy: user.id,
            assignedManagerUserId: createForm.assignedManagerUserId || user.id,
            baseResume: getEmptyBaseResume(),
          }),
        },
        token,
      );
  ...
  async function handleDeleteProfile() {
    ...
    if (!canManageSelectedProfile) {
      setError("Only admins or the assigned manager can delete this profile.");
      setConfirmDeleteOpen(false);
      return;
    }
...
function normalizeStringList(value: unknown, options?: NormalizeTextOptions): string[] {
  if (!Array.isArray(value)) return [""];
  const cleaned = value.map((item) => cleanString(item as string | number | null, options));
  return cleaned.length ? cleaned : [""];
}

function normalizeWorkExperience(value: unknown, options?: NormalizeTextOptions): WorkExperience {
  const source = isPlainObject(value) ? value : {};
  return {
    companyTitle: cleanString(source.companyTitle as string | number | null, options),
    roleTitle: cleanString(source.roleTitle as string | number | null, options),
    employmentType: cleanString(source.employmentType as string | number | null, options),
    location: cleanString(source.location as string | number | null, options),
    startDate: cleanString(source.startDate as string | number | null, options),
    endDate: cleanString(source.endDate as string | number | null, options),
    bullets: normalizeStringList(source.bullets, options),
  };
}

function normalizeEducation(value: unknown, options?: NormalizeTextOptions): EducationEntry {
  const source = isPlainObject(value) ? value : {};
  return {
    institution: cleanString(source.institution as string | number | null, options),
    degree: cleanString(source.degree as string | number | null, options),
    field: cleanString(source.field as string | number | null, options),
    date: cleanString(source.date as string | number | null, options),
    coursework: normalizeStringList(source.coursework, options),
  };
}
...
function normalizeBaseResume(value?: BaseResume, options?: NormalizeTextOptions): BaseResume {
  ...
  const workExperience =
    Array.isArray(value.workExperience) && value.workExperience.length
      ? value.workExperience.map((item) => normalizeWorkExperience(item, options))
      : [getEmptyWorkExperience()];
  const education =
    Array.isArray(value.education) && value.education.length
      ? value.education.map((item) => normalizeEducation(item, options))
      : [getEmptyEducation()];
  const skillsInput = isPlainObject(value.skills) ? value.skills : {};

  return {
    Profile: {
      name: cleanString(profileInput.name as string | number | null, options),
      headline: cleanString(profileInput.headline as string | number | null, options),
      contact: {
        location: cleanString(contactInput.location as string | number | null, options),
        email: cleanString(contactInput.email as string | number | null, options),
        phone: cleanString(contactInput.phone as string | number | null, options),
        linkedin: cleanString(contactInput.linkedin as string | number | null, options),
      },
    },
    summary: { text: cleanString(summaryInput.text as string | number | null, options) },
    workExperience,
    education,
    skills: { raw: normalizeStringList(skillsInput.raw, options) },
  };
}

function normalizeBaseResumeDraft(value?: BaseResume): BaseResume {
  return normalizeBaseResume(value, { trim: false });
}
...
function serializeBaseResumeDraft(value?: BaseResume): string {
  return JSON.stringify(normalizeBaseResumeDraft(value));
}
...
function cleanString(val?: string | number | null, options?: NormalizeTextOptions) {
  if (typeof val === "number") return String(val);
  if (typeof val === "string") return options?.trim === false ? val : val.trim();
  return "";
}

function getUserDisplayName(member?: Pick<User, "email" | "name" | "userName"> | null) {
  return (
    cleanString(member?.userName) ||
...
function getEmptyCreateForm(assignedManagerUserId = "") {
  return {
    displayName: "",
    resumeTemplateId: "",
  ...
  return list
    .map((item, index) => {
      ...
      const meta = [item.location, item.employmentType]
        ...
        .filter(Boolean)
        .join(" | ");
      const bullets = (item.bullets ?? []).map((value) => cleanString(value)).filter(Boolean);
...
function buildEducationHtml(items?: EducationEntry[]) {
  ...
  return list
    .map((item, index) => {
      const title = [item.degree, item.field].map((value) => cleanString(value)).filter(Boolean).join(" - ");
      const header = [item.institution, title].map((value) => cleanString(value)).filter(Boolean).join(" | ");
      const date = cleanString(item.date);
      const coursework = (item.coursework ?? []).map((value) => cleanString(value)).filter(Boolean);
